...
export default class message {
	...
	async run(message: Message) {
		...
		if (!command) return;
		if (command.help.guildOnly && message.channel.type === 'dm') return message.channel.send('Try this command in a server.');
		else if (command.help.nsfw && message.channel.type === 'text' && !message.channel.nsfw)
